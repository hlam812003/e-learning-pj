...
import { courseService, lessonService, CourseImage, Lesson } from '@/features/courses'
...
export default function CourseDetailsPage() {
  ...
  const { course, lessons, isLoading, courseError } = queryResults
    ...
    return (
      <div className="w-full py-20 flex flex-col items-center justify-center text-center">
        ...
        <p className="text-[1.25rem] text-zinc-600 mb-6">
          {courseError instanceof Error ? courseError.message : 'Failed to load course details'}
        </p>
        <button 
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <Icon 
                          key={star}
                          icon={star <= Math.round(rating) ? 'ph:star-fill' : 'ph:star'} 
